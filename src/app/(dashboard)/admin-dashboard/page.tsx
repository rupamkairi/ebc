  ...
  return (
    <div className="flex flex-col gap-8 p-8">
      ...
      <section className="relative overflow-hidden rounded-3xl bg-slate-900 px-8 py-12 text-white shadow-2xl">
        <div className="relative z-10 max-w-2xl">
          ...
          <p className="mb-8 text-lg text-slate-300">
            Monitor your marketplace performance, manage users, and keep track of your business growth in real-time.
          </p>
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
        {stats.map((stat) => (
          <Card key={stat.title} className="border-none shadow-md transition-all hover:shadow-xl hover:-translate-y-1">
            ...
            <CardContent>
              ...
              <p className="mt-1 flex items-center text-xs">
                ...
                <span className="ml-1 text-muted-foreground text-[10px] uppercase font-semibold">from last month</span>
              </p>
            </CardContent>
          </Card>
        ))}
        ...
        <Card className="lg:col-span-4 border-none shadow-md overflow-hidden">
          <CardHeader className="flex flex-row items-center justify-between border-b bg-slate-50/50 dark:bg-slate-900/50 px-6 py-4">
            <div>
              ...
              <p className="text-sm text-muted-foreground">Monthly sales and growth indicators</p>
            </div>
          ...
          <CardContent className="p-6">
            <div className="h-[300px] w-full bg-slate-50 dark:bg-slate-900/50 rounded-2xl flex items-center justify-center border-2 border-dashed border-slate-200 dark:border-slate-800">
              <div className="text-center">
                ...
                <p className="text-slate-400 font-medium font-sans">Interactive Chart Integration Coming Soon</p>
                <div className="mt-6 flex items-end gap-2 justify-center">
                  {[40, 70, 45, 90, 65, 80, 50, 85, 60, 95].map((h, i) => (
                    <div 
                      key={i} 
                      className={`w-6 rounded-t-lg transition-all duration-500 hover:brightness-110 ${i % 2 === 0 ? 'bg-blue-500/30' : 'bg-purple-500/30'}`}
                      style={{ height: `${h}px` }}
                    />
                  ))}
                </div>
            ...
            <CardContent>
              <div className="space-y-5">
                {recentActivity.map((item, i) => (
                  <div key={i} className="group flex items-start gap-4 transition-all hover:translate-x-1">
                    <div className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl text-xs font-bold shadow-sm transition-transform group-hover:scale-110 ${item.color}`}>
                      {item.avatar}
                    </div>
                ...
                <Button variant="ghost" className="w-full mt-2 text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-900/10">
                  View All Activity
                </Button>
              </div>
            </CardContent>
            ...
            <CardContent className="p-4">
              <div className="flex items-center justify-between">
                ...
                <div className="flex gap-1">
                  {[1, 2, 3, 4, 5].map((i) => (
                    <div key={i} className="h-4 w-1 bg-emerald-500 rounded-full animate-pulse" style={{ animationDelay: `${i * 150}ms` }} />
                  ))}
                </div>
              </div>
            </CardContent>
