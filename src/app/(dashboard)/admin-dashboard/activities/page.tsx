...
export default function AdminActivitiesOverviewPage() {
  ...
  const activities = useMemo(() => {
    ...
    enquiries.forEach(enq => items.push({
      id: enq.id,
      type: "ENQUIRY",
    ...
    appointments.forEach(app => items.push({
      id: app.id,
      type: "APPOINTMENT",
    ...
    quotations.forEach(quo => items.push({
      id: quo.id,
      type: "QUOTATION",
  ...
  if (loadingEnquiries || loadingAppointments || loadingQuotations || loadingVisits) {
    return (
    ...
    <Container className="py-8 space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <Card className="bg-blue-50/30 border-blue-100">
          <CardContent className="pt-6">
            <div className="flex flex-col gap-1">
              <span className="text-sm font-bold text-blue-600 uppercase tracking-widest">Enquiries</span>
              <span className="text-4xl font-black text-blue-900">{enquiries.length}</span>
            </div>
          </CardContent>
        </Card>
        <Card className="bg-purple-50/30 border-purple-100">
          <CardContent className="pt-6">
            <div className="flex flex-col gap-1">
              <span className="text-sm font-bold text-purple-600 uppercase tracking-widest">Appointments</span>
              <span className="text-4xl font-black text-purple-900">{appointments.length}</span>
            </div>
          </CardContent>
        </Card>
        <Card className="bg-emerald-50/30 border-emerald-100">
          <CardContent className="pt-6">
            <div className="flex flex-col gap-1">
              <span className="text-sm font-bold text-emerald-600 uppercase tracking-widest">Quotations</span>
              <span className="text-4xl font-black text-emerald-900">{quotations.length}</span>
            </div>
          </CardContent>
        </Card>
        <Card className="bg-amber-50/30 border-amber-100">
          <CardContent className="pt-6">
            <div className="flex flex-col gap-1">
              <span className="text-sm font-bold text-amber-600 uppercase tracking-widest">Visits</span>
              <span className="text-4xl font-black text-amber-900">{visits.length}</span>
            </div>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="space-y-4 max-w-4xl">
        ...
        <div className="relative space-y-0">
          ...
          {activities.map((activity, index) => (
            <div key={`${activity.type}-${activity.id}-${index}`} className="relative pl-12 pb-8 group">
              <div className={cn(
                ...
                {activity.type === "ENQUIRY" && <MessageSquare size={10} className="text-white" />}
                {activity.type === "APPOINTMENT" && <Calendar size={10} className="text-white" />}
                {activity.type === "QUOTATION" && <FileText size={10} className="text-white" />}
                {activity.type === "VISIT" && <MapPin size={10} className="text-white" />}
              </div>
                  ...
                  <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                    <div className="space-y-1">
                      ...
                      <p className="text-sm font-medium text-primary/60">
                        {activity.userName} <span className="text-primary/30 mx-1">—</span> {activity.subtitle}
                      </p>
                    </div>
                    ...
                    <div className="flex items-center gap-3 w-full sm:w-auto justify-between sm:justify-end">
                      <Badge className={cn(
                        ...
                        activity.type === "ENQUIRY" && "bg-blue-100 text-blue-700",
                        activity.type === "APPOINTMENT" && "bg-purple-100 text-purple-700",
                        activity.type === "QUOTATION" && "bg-emerald-100 text-emerald-700",
                        activity.type === "VISIT" && "bg-amber-100 text-amber-700",
                      )}>
                        {activity.status}
                      </Badge>
                      <Button
          ...
          {activities.length === 0 && (
            <div className="py-20 text-center">
              <p className="text-muted-foreground font-bold italic">No activities recorded yet.</p>
            </div>
          )}
        </div>
      </div>
