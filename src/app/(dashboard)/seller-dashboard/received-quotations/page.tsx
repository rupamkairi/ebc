...
export default function SellerReceivedQuotationsPage() {
  ...
  // For sellers, we only want to show quotations they RECEIVED for their enquiries.
  // The backend already filters this via the OR clause, but we need to make sure 
  // we are not showing quotations they SENT here.
  // Actually, the "Received Quotes" page should probably only show quotations
  // where the enquiry was created by the current user.
  const filtered = (quotations || []).filter((q) => {
    // Only show quotations received for enquiries created by the user
    if (q.enquiry?.createdById !== session?.user?.id) return false;
    ...
    <div className="flex flex-col gap-10 w-full max-w-7xl mx-auto">
      ...
      {session?.user && (
        <BuyerProfileCard
          ...
          settingsHref="/seller-dashboard/settings"
                  ...
                  <div className="flex flex-col md:flex-row">
                    ...
                    <div className="flex-1 p-5 md:p-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
                      ...
                      <div className="flex items-center gap-4 md:gap-6">
                        ...
                        >
                          <Link
                            href={`/seller-dashboard/received-quotations/${q.id}`}
                          >
                            Details
