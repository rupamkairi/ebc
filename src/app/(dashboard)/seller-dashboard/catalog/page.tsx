...
export default function CatalogPage() {
  ...
  // 3. Logic: Lock tabs based on Business Operation Type
  const isServiceBusiness = sellerEntity?.op_type === "SERVICE";
  const isProductBusiness = sellerEntity?.op_type === "PRODUCT";

  const [activeTab, setActiveTab] = useState("products");

  // Sync tab when data loads
  useEffect(() => {
    if (sellerEntity) {
      if (isServiceBusiness) setActiveTab("services");
      else if (isProductBusiness) setActiveTab("products");
    }
  }, [sellerEntity, isServiceBusiness, isProductBusiness]);

  // 4. Filter listings based on type (PRODUCT/SERVICE)
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 bg-white p-8 md:p-12 rounded-[2.5rem] shadow-xl shadow-black/5 border border-white relative overflow-hidden">
          ...
          <div className="space-y-2 relative z-10">
            ...
            <p className="text-foreground/60 font-bold italic">
              {sellerEntity ? `Managing ${isServiceBusiness ? 'service offerings' : 'product listings'} for ${sellerEntity.name}` : "Showcase your professional catalog to verified buyers."}
            </p>
          </div>
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
            <TabsList className="bg-white border border-border p-1.5 rounded-2xl h-auto self-start shadow-sm flex overflow-hidden">
              <TabsTrigger 
                ...
                className="rounded-xl px-10 py-3 font-black text-xs data-[state=active]:bg-primary data-[state=active]:text-white data-[state=active]:shadow-lg transition-all uppercase tracking-widest border-none outline-none disabled:opacity-30 disabled:cursor-not-allowed"
              >
                Products
              </TabsTrigger>
              <TabsTrigger 
