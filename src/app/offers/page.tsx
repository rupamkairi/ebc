...
import { CheckCircle2, ChevronRight, Tag, Calendar, MapPin, User, Search, Filter, ShieldCheck } from "lucide-react";
        ...
        {options.map((opt) => (
          <label key={opt.id} className="flex items-center gap-3 group cursor-pointer">
            ...
            <span className={cn(
              ...
              opt.checked ? "text-slate-900" : "text-slate-400 group-hover:text-slate-600"
            )}>
              {opt.label}
            </span>
          </label>
...
function Sidebar({ selectedCategories, onCategoryToggle }: { selectedCategories: string[], onCategoryToggle: (id: string) => void }) {
...
function OfferCard({ offer }: { offer: Offer }) {
  ...
  const locationText = location ? `${location.district}, ${location.state}` : "West Bengal, India";
    ...
    <div className="bg-[#445EB4] rounded-2xl overflow-hidden flex flex-col group transition-all duration-500 hover:scale-[1.02] hover:shadow-2xl hover:shadow-[#445EB4]/30 shadow-lg h-full">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col min-w-0">
            <div className="flex items-center gap-1.5">
              <span className="text-white text-[11px] font-black uppercase tracking-tight truncate">{offer.entity?.name || "ABC Supplies"}</span>
              {isVerified && <CheckCircle2 className="size-3 text-[#4ade80] fill-[#4ade80]/20 shrink-0" />}
            </div>
      ...
      <div className="p-5 flex flex-col gap-4 grow">
          ...
          <div className="space-y-1.5">
            {detail?.startDate && (
              <div className="flex items-center gap-2 text-[9px] font-bold text-white/50 uppercase tracking-widest">
                ...
                <span>Valid till {detail.endDate ? format(new Date(detail.endDate), "dd/MM/yyyy") : "18/01/2026"}</span>
              </div>
            )}
            <div className="text-[9px] font-bold text-[#FFA500]/70 uppercase tracking-widest">
               Ends in {(offer.id.split("").reduce((acc, char) => acc + char.charCodeAt(0), 0) % 28) + 2} Days
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 text-[10px] text-white/80 font-bold bg-white/5 px-3 py-2 rounded-lg border border-white/5">
           ...
           <span className="truncate uppercase tracking-tight">Recommended for G+1 Houses</span>
        </div>
        ...
        <div className="space-y-2 mt-auto">
          ...
          <Button variant="ghost" className="w-full border border-white/20 text-white hover:bg-white/10 hover:text-white font-black rounded-lg h-10 text-[10px] uppercase tracking-[0.1em] transition-all">
            Ask a Question
          </Button>
        </div>
      </div>
    ...
    <div className="py-12 text-center space-y-8">
      <div className="space-y-4">
        <h1 className="text-4xl md:text-5xl font-black tracking-tight">
          <span className="text-[#445EB4]">EBC</span> <span className="text-[#FFA500]">Conference Hall</span> <span className="text-[#445EB4]">Offers</span>
        </h1>
        <p className="text-slate-500 font-medium text-lg max-w-2xl mx-auto italic">
          &ldquo;Limited-time offers from verified sellers &amp; service providers near you&rdquo;
        </p>
      </div>
...
function OffersPageContent() {
  ...
  const filteredOffers = offers?.filter(offer => {
    ...
    return offer.offerRelations.some(rel => 
      rel.relationType === "CATEGORY" && rel.relationId && selectedCategories.includes(rel.relationId)
    );
  }) || [];
          ...
          <div className="flex-1 space-y-16">
            ...
            <section className="space-y-8">
              <div className="flex items-center gap-4">
                <h2 className="text-2xl md:text-3xl font-black text-[#445EB4] tracking-tight">
                  Today&apos;s <span className="text-[#FFA500]">Offers</span> Near You
                </h2>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
                  ...
                  {filteredOffers.length === 0 && (
                    <div className="col-span-full py-20 text-center bg-white border border-dashed rounded-3xl border-slate-200">
                      <p className="text-slate-400 font-black uppercase tracking-widest">No offers found today</p>
                    </div>
                  )}
                </div>
              )}
            ...
            <div className="bg-[#FFF9F0] border border-[#FFE4B5] rounded-2xl p-6 flex flex-col md:flex-row items-center justify-center gap-4 text-center md:text-left">
              ...
              <p className="text-slate-600 font-bold text-sm">
                <span className="text-[#FFA500]">Offers on EBC</span> are reviewed by Our team, always compare and decide Confidently
              </p>
            </div>
...
export default function OffersPage() {
  return (
    <Suspense fallback={<div className="h-screen flex items-center justify-center font-black text-2xl text-[#445EB4] animate-pulse italic">EBC LOADING...</div>}>
      <OffersPageContent />
    </Suspense>
  );
}
