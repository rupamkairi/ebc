      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <div className="relative w-full md:max-w-md">
          <input
            ...
            className="w-full h-11 pl-5 pr-14 rounded-full border border-slate-200 bg-white shadow-sm focus:outline-none focus:ring-2 focus:ring-[#FFA500]/40 transition-all text-slate-700 font-medium placeholder:text-slate-400"
            value={searchTerm}
            onChange={(e) => setSearchTerm(e.target.value)}
          />
          <button className="absolute right-0 top-0 h-11 w-12 flex items-center justify-center bg-[#FFA500] hover:bg-[#E69500] rounded-r-full text-white transition-all active:scale-95">
        ...
        <div className="flex items-stretch h-10 rounded-xl overflow-hidden shadow-md border border-white/10 bg-[#1e274f]">
          ...
          >
            <SelectTrigger className="h-full px-4 bg-transparent border-none rounded-none text-white font-semibold text-sm focus:ring-0 shadow-none gap-2.5 hover:bg-white/10 transition-colors [&>span]:flex [&>span]:items-center [&>span]:gap-2">
              <ArrowUpDown className="size-3.5 text-[#FFA500] shrink-0" />
          ...
          <Sheet>
            <SheetTrigger asChild>
              <button className="flex items-center gap-2.5 px-4 h-full text-white hover:bg-white/10 transition-colors">
                <ClipboardList className="size-4 text-[#FFA500] shrink-0" />
                <span className="text-[10px] uppercase tracking-widest text-white/50 font-bold hidden xs:inline">Enquiry</span>
                <span className="min-w-[20px] h-5 px-1.5 bg-[#FFA500] text-white text-xs font-black rounded-full flex items-center justify-center tabular-nums leading-none">
                  {inquiryCount}
                </span>
              </button>
            </SheetTrigger>
          ...
          <Sheet>
            <SheetTrigger asChild>
              <button className="flex items-center gap-2 px-4 h-full text-white hover:bg-white/10 transition-colors lg:hidden">
                <SlidersHorizontal className="size-4 text-[#FFA500]" />
