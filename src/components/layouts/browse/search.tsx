...
import { BrowseType, useBrowseParams } from "@/hooks/useBrowseParams";
    ...
    <div className="flex flex-col gap-5 w-full">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <div className="relative w-full md:max-w-md">
          <input
            type="text"
            placeholder="Search construction needs..."
          ...
          <button
            type="button"
            className="absolute right-0 top-0 h-11 w-12 flex items-center justify-center bg-secondary hover:bg-secondary/90 rounded-r-full text-white transition-all active:scale-95"
          >
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
        ...
        <div className="flex flex-wrap items-center gap-3">
          <Tabs
            value={params.type}
            onValueChange={(value) =>
              updateParams({ type: value as BrowseType })
            }
          >
            <TabsList className="h-10 bg-white border border-slate-200 shadow-sm">
              <TabsTrigger value="ALL" className="px-4">
                All
              </TabsTrigger>
              <TabsTrigger value="PRODUCT" className="px-4">
                Materials
              </TabsTrigger>
          ...
          <div className="flex items-stretch h-10 rounded-xl overflow-hidden shadow-md border border-white/10 bg-[#1e274f]">
            {/* Sort By — SelectTrigger IS the button - This Does not make sense in this business. */}
            {/* <Select
              value={params.sort}
            ...
            <Sheet>
              <SheetTrigger asChild>
                <button
                  type="button"
                  className="flex items-center gap-2 px-4 h-full text-white hover:bg-white/10 transition-colors lg:hidden"
                >
