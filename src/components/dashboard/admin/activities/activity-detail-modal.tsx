          ...
          <div className="p-6 space-y-8">
            ...
            <section className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 bg-slate-50 p-4 rounded-xl border border-slate-100">
                <div className="space-y-1">
                  ...
                  <p className="text-sm font-black text-primary">{createdBy?.name || "N/A"}</p>
                </div>
                <div className="space-y-1">
            ...
            <section className="space-y-4">
              ...
              <div className="space-y-3">
                {type === "ENQUIRY" ? (
                  (activity as Enquiry).enquiryLineItems.map((item: EnquiryLineItem, idx) => (
                    <div key={idx} className="flex justify-between items-start p-3 rounded-lg border bg-white shadow-sm">
                      <div className="space-y-1">
                        <p className="text-sm font-bold text-primary">{item.item?.name}</p>
                        <p className="text-xs text-muted-foreground">{item.remarks || "No remarks"}</p>
                      </div>
                      <div className="text-right">
                        <p className="text-sm font-black text-primary">{item.quantity} {item.unitType}</p>
                      </div>
                    </div>
                  ))
                ) : (
                  (activity as Appointment).appointmentLineItems.map((item: AppointmentLineItem, idx) => (
                    <div key={idx} className="flex justify-between items-start p-3 rounded-lg border bg-white shadow-sm">
                      <div className="space-y-1">
                        <p className="text-sm font-bold text-primary">{item.item?.name}</p>
                        <p className="text-xs text-muted-foreground">{item.remarks || "No remarks"}</p>
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-2">
                    <p className="text-sm font-medium text-primary">
                      {(activity as Enquiry).enquiryDetails?.[0]?.address || "No address provided"}
                    </p>
                  </div>
                </div>
              ) : (
              ...
              <div className="space-y-4">
                ...
                <p className="text-sm font-medium text-primary italic">
                  {type === "ENQUIRY" 
                    ...
                    : (activity as Appointment).appointmentDetails?.[0]?.remarks || "None"}
                </p>
              </div>
            </section>
            ...
            <section className="space-y-4">
              <h3 className="text-xs font-black uppercase tracking-widest text-muted-foreground flex items-center gap-2">
                {type === "ENQUIRY" ? <CheckCircle2 size={14} /> : <Clock size={14} />}
                {type === "ENQUIRY" ? "Seller Responses (Quotations)" : "Provider Responses (Visits)"}
              </h3>
              ...
              {type === "ENQUIRY" ? (
                <div className="space-y-4">
                  {quotations.length === 0 ? (
                    <div className="flex flex-col items-center justify-center py-8 text-muted-foreground bg-slate-50 rounded-xl border border-dashed">
                      ...
                      <p className="text-sm font-bold">No quotations received yet.</p>
                    </div>
                  ) : (
                    quotations.map((quo, idx) => (
                      <div key={idx} className="p-4 rounded-xl border bg-white shadow-sm space-y-3">
                        <div className="flex justify-between items-center">
                          <div className="flex items-center gap-2">
                            <p className="text-sm font-black text-primary">{quo.createdBy?.name || "Seller"}</p>
                        ...
                        <div className="grid grid-cols-2 gap-4 text-sm">
                          <div className="p-2 bg-emerald-50 rounded-lg">
                            <p className="text-[10px] font-black text-emerald-600 uppercase">Rate</p>
                            <p className="font-black text-emerald-700">₹ {quo.quotationLineItems[0]?.rate || 0}</p>
                          </div>
                          <div className="p-2 bg-slate-50 rounded-lg">
                            <p className="text-[10px] font-black text-slate-500 uppercase">Remarks</p>
                ...
                <div className="space-y-4">
                  {!visit ? (
                    <div className="flex flex-col items-center justify-center py-8 text-muted-foreground bg-slate-50 rounded-xl border border-dashed">
                      <AlertCircle size={24} className="mb-2 opacity-20" />
                      <p className="text-sm font-bold">No visit scheduled yet.</p>
                    </div>
                  ) : (
                    <div className="p-4 rounded-xl border bg-white shadow-sm space-y-4">
                      <div className="flex justify-between items-center">
                        <div className="flex items-center gap-2">
                          <p className="text-sm font-black text-primary">{visit.createdBy?.name || "Provider"}</p>
                          <Badge className={cn(
                        ...
                        <p className="text-xs text-muted-foreground">
                          Scheduled: {format(new Date(visit.visitSlot?.fromDateTime || ""), "MMM dd, yyyy")}
                        </p>
                      </div>
                      <div className="p-3 bg-blue-50 rounded-lg flex items-center justify-between">
                        <div className="space-y-1">
                          <p className="text-[10px] font-black text-blue-600 uppercase tracking-widest">Time Slot</p>
                          <p className="text-xs font-black text-blue-800">
                            {format(new Date(visit.visitSlot?.fromDateTime || ""), "hh:mm a")} - {format(new Date(visit.visitSlot?.toDateTime || ""), "hh:mm a")}
                          </p>
                        </div>
                        <div className="text-right space-y-1">
                          <p className="text-[10px] font-black text-blue-600 uppercase tracking-widest">Type</p>
                          <p className="text-xs font-black text-blue-800">Physical Visit</p>
                        </div>
                      </div>
                    </div>
                  )}
