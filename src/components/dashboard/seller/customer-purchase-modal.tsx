          ...
          <div className="relative flex items-center gap-5">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-3 mt-1">
                <Badge className="bg-secondary text-white border-none  text-[9px] font-black px-3 py-0.5 rounded">
                  {customer.status}
                </Badge>
            ...
            <div className="space-y-6 pb-4">
              {sortedActivities.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-12 text-center bg-white rounded-2xl border-2 border-dashed border-slate-100 italic text-slate-400 text-sm">
                  {t(
                    "no_interactions_recorded",
                    "No service interactions recorded yet.",
                  )}
                </div>
              ) : (
                <div className="space-y-4">
                  {sortedActivities.map((act) => {
                    ...
                    const items = isEnquiry
                      ? act.enquiryLineItems
                      : act.appointmentLineItems;
                    const details = isEnquiry
                      ? act.enquiryDetails?.[0]
                      : act.appointmentDetails?.[0];

                    return (
                      <div
                        key={act.id}
                        className="bg-white rounded-[20px] border border-slate-100 shadow-sm overflow-hidden transition-all hover:shadow-md"
                      >
                        {/* Entry Sub-header */}
                        <div className="bg-slate-50/50 px-5 py-3 flex items-center justify-between border-b border-slate-100">
                          <div className="flex items-center gap-2">
                            ...
                            <div>
                              <p className="text-[9px] font-black text-slate-400  tracking-tight leading-none mb-0.5">
                                {isEnquiry
                                  ? t("enquiry", "Enquiry")
                                  : t("appointment", "Appointment")}
                              </p>
                              <p className="text-[11px] font-black text-primary leading-none">
                                #{act.id.substring(0, 8).toUpperCase()}
                              </p>
                            </div>
                          </div>
                        ...
                        <div className="p-4 space-y-4">
                          <div className="space-y-2">
                            {items.map((item, idx) => (
                              <div
                                key={idx}
                                className="flex items-center justify-between text-[13px] bg-slate-50/50 p-2.5 rounded-xl border border-slate-50"
                              >
                                ...
                                <div className="text-right">
                                  <span className="font-black text-primary">
                                    {isEnquiry
                                      ? (item as EnquiryLineItem).quantity
                                      : 1}
                                  </span>
                        ...
                        <div className="px-5 py-2 bg-slate-50/30 flex justify-end border-t border-slate-50">
                          <Button
                            variant="ghost"
                            size="sm"
                            className="h-7 text-[10px] font-black  text-primary hover:bg-primary/5 transition-all"
                            asChild
                          >
                            <a
                              href={`/seller-dashboard/${isEnquiry ? "enquiries" : "appointments"}/${act.id}`}
                            >
