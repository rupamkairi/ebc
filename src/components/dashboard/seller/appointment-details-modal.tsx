      ...
      <DialogContent className="sm:max-w-2xl p-0! gap-0! overflow-hidden border-none rounded-4xl bg-white shadow-2xl max-h-[90vh] flex flex-col">
        ...
        <DialogHeader className="p-6 pb-2 shrink-0">
          <div className="flex items-center justify-between mb-1">
            <span className="text-[10px] font-black  tracking-[0.2em] bg-muted px-3 py-1 rounded-full text-foreground/40">
              APT {apt.id.slice(-8).toUpperCase()}
            </span>
            <Badge
              className={cn(
                " tracking-tighter font-black text-[10px] rounded-full px-3 py-1 border-none",
                apt.status === "PENDING"
              ...
              <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
                ...
                <div className="md:col-span-2 bg-muted/10 p-4 rounded-3xl border border-muted/20 space-y-2">
                  <div className="flex items-center gap-2 text-primary">
                    <Package size={16} />
                    <h4 className="font-black italic  text-[10px] ">
                      Service Item
                    </h4>
                  </div>
                  <div className="space-y-0.5">
                    <p className="text-lg font-black italic truncate">
                      {firstItem?.item?.name || "Consultation"}
                    </p>
                    <p className="text-xs text-foreground/40 font-bold italic line-clamp-1">
                      {firstItem?.remarks || "No additional remarks"}
                    </p>
                  </div>
                </div>
                  ...
                  <div className="grid grid-cols-2 gap-x-4 gap-y-2">
                    <div className="flex items-center gap-2">
                      ...
                      <p className="text-xs font-black italic truncate">
                        {apt.createdBy?.phone || "Phone Hidden"}
                      </p>
                    </div>
                    <div className="flex items-center gap-2">
                      <Mail size={12} className="text-primary shrink-0" />
                      <p className="text-xs font-black italic truncate">
                        {apt.createdBy?.email || "No Email"}
                      </p>
                    </div>
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        >
                          ...
                          <span className="">
                            {format(new Date(slot.fromDateTime), "MMM")}
                          </span>
                        </div>
          ...
          apt.status === "COMPLETED" ? (
            ...
            <div className="flex flex-col gap-4">
              <Button
                ...
                disabled={
                  isConfirming || isRejecting || !slots[selectedSlotIdx]?.id
                }
              ...
              >
                {isConfirming ? (
                  ...
                  "Proceed to payment and review"
                )}
              </Button>
              <div className="flex items-center justify-between px-2 font-black   text-[10px]">
                <button
