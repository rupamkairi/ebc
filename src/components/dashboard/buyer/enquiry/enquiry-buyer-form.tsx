...
interface EnquiryBuyerFormProps {
  defaultValues?: Partial<BuyerDetails> | null;
  onChange: (values: BuyerDetails) => void;
}

export function EnquiryBuyerForm({
  defaultValues,
  onChange,
}: EnquiryBuyerFormProps) {
  ...
  useEffect(() => {
    if (session?.user && !defaultValues) {
      form.reset({
        ...
        // If we have a way to get pincode from user, we should add it here
      });
    }
  ...
  const inputClass =
    "h-14 bg-white dark:bg-white border-[#3D52A0]/20 rounded-xl focus:border-[#FFA500] focus:ring-[#FFA500]/10 font-medium text-[#3D52A0] transition-all duration-200 placeholder:text-[#3D52A0]/20";
  const labelClass =
    "text-[10px] font-black tracking-widest text-[#3D52A0] uppercase opacity-60 ml-1";
      ...
      <div className="relative z-10 space-y-10">
        <div className="space-y-4">
          <div className="flex items-center gap-4">
            ...
            <h2 className="text-3xl font-black italic tracking-tighter text-[#3D52A0]">
              Your Details
            </h2>
          </div>
          <div className="h-1 w-32 bg-linear-to-r from-[#3D52A0] to-transparent rounded-full" />
        </div>
                    ...
                    <FormControl>
                      <div className="relative">
                        ...
                        <PincodeSearchAutocomplete
                          ...
                          className={cn(
                            inputClass,
                            "pl-12 z-20 relative bg-transparent",
                          )}
                        />
                      </div>
                    </FormControl>
                  ...
                  <FormControl>
                    <div className="relative">
                      ...
                      <Textarea
                        ...
                        className={cn(
                          inputClass,
                          "min-h-[120px] pl-12 py-4 resize-none",
                        )}
                      />
                    </div>
                  </FormControl>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem className="space-y-1">
                    ...
                    <FormControl>
                      <div className="relative">
                        <Calendar className="absolute left-4 top-1/2 -translate-y-1/2 size-4 text-[#3D52A0]/30" />
                        <Input
                          type="date"
                          {...field}
                          className={cn(inputClass, "pl-12")}
