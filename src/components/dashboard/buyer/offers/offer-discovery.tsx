...
export function OfferDiscovery() {
  ...
  const activeOffers = offers?.filter(o => o.isActive && o.offerDetails?.[0]?.publishedAt) || [];
      ...
      {activeOffers.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {activeOffers.map((offer) => {
            ...
            return (
              <Card key={offer.id} className="group relative border-none bg-linear-to-br from-card via-card to-primary/5 overflow-hidden rounded-4xl shadow-xl hover:shadow-2xl hover:shadow-primary/5 transition-all duration-500">
                ...
                <CardHeader className="pb-4">
                  <div className="mb-4">
                     <Badge className="bg-primary text-primary-foreground font-black px-3 py-1 rounded-lg shadow-lg shadow-primary/20">
                      PROMOTION
                     </Badge>
                  </div>
                  <CardTitle className="text-2xl font-black leading-tight group-hover:text-primary transition-colors">{offer.name}</CardTitle>
                  ...
                  <div className="p-4 bg-muted/30 rounded-2xl space-y-3">
                    {detail?.startDate && (
                      <div className="flex items-center gap-2.5 text-xs font-bold text-muted-foreground">
                        ...
                        <span>VALID: {format(new Date(detail.startDate), "PP")} - {detail.endDate ? format(new Date(detail.endDate), "PP") : "Ongoing"}</span>
                      </div>
                    )}
                  </div>
                </CardContent>
                ...
                <CardFooter className="pt-2 pb-8 flex flex-col gap-4">
                  <Button className="w-full h-12 rounded-xl font-black gap-2 group/btn" asChild>
                    <a href={`/buyer-dashboard/conference-hall/offers/${offer.id}`}>
                      VIEW OFFER DETAILS
                  ...
                  <div className="flex items-center justify-center gap-2 text-[10px] font-black text-muted-foreground uppercase tracking-widest">
                    <span>HOSTED BY</span>
                    <span className="text-foreground">{offer.entity?.name}</span>
                  </div>
                </CardFooter>
              </Card>
            );
